...
function getCss(node,prop) {
  if(typeof node === 'string') node = getNode(node);
  if(!(prop in document.body.style)) syntaxError('getCss 함수의 두 번째 인자 prop은 유효한 css 속성이 아닙니다');
  return getComputedStyle(node)[prop]
}

function setCss(node,prop,value) {
  if(typeof node === 'string') node = getNode(node);
  if(!(prop in document.body.style)) syntaxError('setCss 함수의 두 번째 인자인 prop은 유효한 css 속성이 아닙니다');
  if(!value) syntaxError('setCss 함수의 세 번째 인자는 필수 입니다');
